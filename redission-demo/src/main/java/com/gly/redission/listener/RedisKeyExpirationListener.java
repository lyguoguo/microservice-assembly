...
public class RedisKeyExpirationListener extends KeyExpirationEventMessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        if(expiredKey.startsWith(redisKey)){
            ...
            boolean lock = redisLock.tryLock(String.format("redisson:lock:%s",expiredKey),TimeUnit.SECONDS,0,-1);
            if(!lock){
            ...
            redisLock.unlock(String.format("redisson:lock:%s",expiredKey));
            logger.info("key:{}锁已释放",expiredKey);
        }
    }
